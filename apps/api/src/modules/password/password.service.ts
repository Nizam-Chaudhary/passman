...
class PasswordService {
  ...
  async getPasswords(userId: number, vaultId: number, search?: string) {
    ...
    const passwordsData = await db.query.passwords.findMany({
      where: and(eq(passwords.userId, userId), eq(passwords.vaultId, vaultId), searchCondition),
  ...
  async updatePassword(id: number, input: UpdatePasswordBody, userId: number) {
  ...
  async movePasswordsToVault(userId: number, body: MoveMultiplePasswordsVaultBody) {
    await db
      .update(passwords)
      .set({ vaultId: body.vaultId })
      .where(and(inArray(passwords.id, body.ids), eq(passwords.userId, userId)));
