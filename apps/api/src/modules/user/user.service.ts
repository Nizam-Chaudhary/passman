...
class UserService {
  ...
  async createMasterKey(id: number, input: CreateMasterKeyBody) {
    const hashedMasterPassword = await Bun.password.hash(input.masterPassword);
  ...
  async resetPassword(body: ResetPasswordBody) {
    ...
    const [passwordUpdate] = await db
      ...
      .where(like(users.email, jwtTokenResult.payload.email.toLowerCase()));
  ...
  async updateMasterPassword(userId: number, body: UpdateMasterPasswordBody) {
    const hashedMasterPassword = await Bun.password.hash(body.masterPassword);
