...
interface VerifyTokenResult<T> {
  ...
  payload?: T;
  error?: {
    code: "INVALID_TOKEN" | "TOKEN_EXPIRED" | "INVALID_SIGNATURE" | "MALFORMED_TOKEN";
    message: string;
  };
}
...
export async function createToken<T extends Record<string, any>>(options: CreateToken<T>) {
...
export function decodeToken<T = Record<string, any>>(token: string): DecodeTokenResult<T> {
  try {
...
function calculateExpirationTime(expiresInSeconds: number) {
  return Math.floor(Date.now() / 1000) + expiresInSeconds;
}

export type { CreateToken, DecodeTokenResult, VerifyTokenResult };
